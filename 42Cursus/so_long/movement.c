...
/*   Updated: 2022/06/27 19:30:32 by chanhale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_position(t_map* map, int move)
{
	...
	if (move == TYPE_MOVE_RIGHT)
		new_pos_y += 1;
	return (ask_and_move(map, new_pos_x, new_pos_y));
}
...
int	ask_and_move(t_map* map, size_t new_pos_x, size_t new_pos_y)
{
	...
	while (object != NULL)
	{
		if(object->x == new_pos_x && object->y == new_pos_y && object->state != TYPE_OBJECT_STATE_COLLECTED)
		{
