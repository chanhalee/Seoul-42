...
/*   Updated: 2022/07/03 21:32:19 by chanhale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_map(char *file_name, t_map *map)
{
	...
	if (fd < 0)
		emergency_exit_bonus(map, TYPE_ERR_CODE_FILE);
	next_line = get_next_line(fd);
	if (next_line == NULL)
		emergency_exit_bonus(map, TYPE_ERR_CODE_FILE);
	map->size_x = ft_strlen(next_line);
	if (map->size_x <= 2)
		emergency_exit_bonus(map, TYPE_ERR_CODE_MAP);
...
void	parse_block(char block, t_map *map, size_t pos_x, size_t pos_y)
{
	if (block == '1')
		...
		forge_ememy(pos_x, pos_y, map);
	else if (block == 'P')
	{
		if (map->user.pos.x != 0 || map->user.pos.y != 0)
			emergency_exit_bonus(map, TYPE_ERR_CODE_MAP);
